node {
    ...
    stage('docker image'){
    sh 'docker build -t nagarajshobha/pipeline:2.0 .'
}
    stage('docker push'){
        withCredentials([string(credentialsId: 'docker-p', variable: 'dockerpwd')]) {
        sh "docker login -u nagarajshobha -p ${dockerpwd}"
}
     sh 'docker push nagarajshobha/pipeline:2.0'
    
}
   stage('ruuning conatiner on develpoer server'){
       def dockerRun = 'docker run --name pipelineapp2 -p 8080:8080 -d nagarajshobha/pipeline:2.0 '
       sshagent(['ssh-con']) {
